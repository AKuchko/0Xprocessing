...
document.prototype.removeClassWild = function (mask) {
  return this.classList.remove(function (index, cls) {
    const re = mask.replace(/\*/g, "\\S+");
    return (cls.match(new RegExp("\\b" + re + "", "g")) || []).join(" ");
  });
};
document.prototype.slideUp = function (duration=500) {
  ...
  this.style.height = target.offsetHeight + 'px'; /* [3] */
...
document.prototype.slideDown = function (duration=500) {
...
class Special {
  ...
  constructor () {
    ...
    if (this.$document.querySelectorAll("#specialButton").length) {
        this.$subversion = "lite";
        if (this.$special && this.$special.active) this.On();
        this.$document.querySelector("#specialButton").addEventListener("click", this.On);
    } else {
        this.$subversion = "pro";
        this.On();
    }
  ...
  setSpecialCookie () {
    this.$document.cookie = `special=${JSON.stringify($special)}; path=/;`;
  }

  removeSpecialCookie () {
    this.$document.cookie = 'special= ;';
  }

  onSpecialClick (event) {
    const specialButton = event.target;
    const parentClass = event.target.parentNode.getAttribute('class').replace('special-', '');
    const defaultAction = () => {
      specialButton.classList.add("active");
      this.$document.querySelector("html").removeClassWild("special-" + parent + "-*").classList.add("special-" + parent + "-" + document.querySelector(this).value);
      this.setSpecialCookie();
    }
    
    if (parentClass) {
      this.$document.querySelector("#special .special-" + parentClass + " button").classList.remove("active");
      switch (parentClass) {
        case "color":
          this.$special.color = parseInt(document.querySelector(this).value);
          defaultAction();
          break;
        case "font-size":
          this.$special.font_size = parseInt(document.querySelector(this).value);
          defaultAction();
          break;
        case "font-family":
          this.$special.font_family = parseInt(document.querySelector(this).value);
          defaultAction();
          break;
        case "line-height":
          this.$special.line_height = parseInt(document.querySelector(this).value);
          defaultAction();
          break;
        case "letter-spacing":
          this.$special.letter_spacing = parseInt(document.querySelector(this).value);
          defaultAction();
          break;
        case "images":
        ...
        case "audio":
          if(specialButton.value == 1) {
            this.$document.querySelector("i.special-audio").remove();
            if (responsiveVoice.isPlaying()) responsiveVoice.cancel();
            this.$document.querySelector("p,h1,h2,h3,h4,h5,h6,li,dt,dd,.audiotext").re();
            this.$document.querySelector(this).value = 0;
          } else {
            responsiveVoice.speak("Включено озвучивание текста.", "Russian Female");
            document.querySelector(this).classList.add("active");
            document.querySelector(this).value = 1;
            document.querySelector("p,h1,h2,h3,h4,h5,h6,li,dt,dd,.audiotext,a,b").addEventListener("mouseover", function () {
              if (responsiveVoice.isPlaying()) responsiveVoice.cancel();
                responsiveVoice.speak(document.querySelector(this).innerText.trim(), "Russian Female");
              }
            );
          }
          break;
        case "settings":
          this.$document.querySelector("#special-settings-body").slideToggle();
          break;
        case "settings-close":
          this.$document.querySelector("#special-settings-body").slideUp();
          break;
        case "reset":
          this.Reset();
          this.Set();
  ...
  Set () {
    this.$document.querySelector("html")
      ...
      .classList.add("special-color-" + $special.color)
      .classList.add("special-font-size-" + $special.font_size)
      .classList.add("special-font-family-" + $special.font_family)
      .classList.add("special-line-height-" + $special.line_height)
      .classList.add("special-letter-spacing-" + $special.letter_spacing)
      .classList.add("special-images-" + $special.images);
    this.$document.querySelector("#special button").classList.remove("active");
    this.$document.querySelector(".special-color button[value=" + $special.color + "]").classList.add("active");
    this.$document.querySelector(".special-font-size button[value=" + $special.font_size + "]").classList.add("active");
    this.$document.querySelector(".special-font-family button[value=" + $special.font_family + "]").classList.add("active");
    this.$document.querySelector(".special-line-height button[value=" + $special.line_height + "]").classList.add("active");
    this.$document.querySelector(".special-letter-spacing button[value=" + $special.letter_spacing + "]").classList.add("active");
    this.$document.querySelector(".special-images button").value = $special.images;

    this.ToggleImages();
  }

  ToggleImages () {
    this.$document.querySelector("img").forEach(el => {
  ...
  Off () {
    if (this.$document.querySelectorAll("#specialButton").length) {
      this.$document.querySelector("html").classList.remove("special").removeClassWild("special-*");
      this.$document.querySelector("i.special-audio").remove();
      if (responsiveVoice.isPlaying()) responsiveVoice.cancel(); // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      this.$document.querySelector("audio").remove();
      this.$document.querySelector("#special").remove();
      this.$document.querySelector("#specialButton").style.display = "block";
  ...
  On () {
    const styleSheet = this.$document.createElement('<link rel="stylesheet" type="text/css" />');
    styleSheet.setAttribute('href', '//lidrekon.ru/slep/css/special.min.css');
    this.$document.querySelector("head").append(styleSheet);

    if (!this.$special) this.Reset();
    if (this.$document.querySelectorAll("#specialButton").length) {
      this.$special.active = 1;
      this.$document.querySelector("#specialButton").style.display = "none";
      this.setSpecialCookie();
    }

    this.$document.querySelector("html").classList.add("special");
    this.$document.querySelector("body").prepend($tpl);
    this.Set();
    this.$document.querySelector("#special button").addEventListener("click", this.onSpecialClick);
  }
}
...
//   const special = {
    ...
    //     images: 1,
    //   };
    //   document.cookie = `special=${JSON.stringify($special)}; path=/;`;
    // },
    ...
    //   document.querySelector(".special-color button[value=" + $special.color + "]").classList.add("active");
    //   document.querySelector(".special-font-size button[value=" + $special.font_size + "]").classList.add("active");
    //   document.querySelector(".special-font-family button[value=" + $special.font_family + "]").classList.add("active");
    //   document.querySelector(".special-line-height button[value=" + $special.line_height + "]").classList.add("active");
    //   document.querySelector(".special-letter-spacing button[value=" + $special.letter_spacing + "]").classList.add("active");
    //   document.querySelector(".special-images button").value = $special.images;

    //   this.ToggleImages();
    // },

    // ToggleImages: function () {
    //   document.querySelector("img").forEach(function () {
    //     if ($special.images) {
    //       if (document.querySelector(this).data("src")) document.querySelector(this).getAttribute("src", document.querySelector(this).data("src"));
    //       if (document.querySelector(this).data("srcset")) document.querySelector(this).getAttribute("srcset", document.querySelector(this).data("srcset"));
    //     } else {
    //       document.querySelector(this).data("src", document.querySelector(this).getAttribute("src"));
    //       if (document.querySelector(this).getAttribute("srcset")) document.querySelector(this).data("srcset", document.querySelector(this).getAttribute("srcset"));
    //       document.querySelector(this).removeAttr("src");
    //       if (document.querySelector(this).getAttribute("srcset")) document.querySelector(this).removeAttr("srcset");
    //     }
    //   });
    // },

    // Off: function () {
    //   if (document.querySelector("#specialButton").length) {
    //     document.querySelector("html").classList.remove("special").removeClassWild("special-*");
    ...
    // On: function () {
    //   document.querySelector("head").insertAdjacentHTML("beforeend", document.querySelector('<link rel="stylesheet" type="text/css" />').getAttribute("href", "//lidrekon.ru/slep/css/special.min.css"));
    ...
    //   document.querySelector("body").prepend($tpl);
    //   special.Set();
    //   document.querySelector("#special button").addEventListener("click", function () {
    //     var parent = document.querySelector(this).parent().getAttribute("class").replace("special-", "");
    //     if (parent) {
    //       document.querySelector("#special .special-" + parent + " button").classList.remove("active");
    //       switch (parent) {
    //         case "color":
    //           $special.color = parseInt(document.querySelector(this).value);
    //           document.querySelector(this).classList.add("active");
    //           document.querySelector("html").removeClassWild("special-" + parent + "-*").classList.add("special-" + parent + "-" + document.querySelector(this).value);
    //           document.cookie = `special=${JSON.stringify($special)}; path=/;`;
    //           break;
    //         case "font-size":
    //           $special.font_size = parseInt(document.querySelector(this).value);
    //           document.querySelector(this).classList.add("active");
    //           document.querySelector("html").removeClassWild("special-" + parent + "-*").classList.add("special-" + parent + "-" + document.querySelector(this).value);
    //           document.cookie = `special=${JSON.stringify($special)}; path=/;`;
    //           break;
    //         case "font-family":
    //           $special.font_family = parseInt(document.querySelector(this).value);
    //           document.querySelector(this).classList.add("active");
    //           document.querySelector("html").removeClassWild("special-" + parent + "-*").classList.add("special-" + parent + "-" + document.querySelector(this).value);
    //           document.cookie = `special=${JSON.stringify($special)}; path=/;`;
    //           break;
    //         case "line-height":
    //           $special.line_height = parseInt(document.querySelector(this).value);
    //           document.querySelector(this).classList.add("active");
    //           document.querySelector("html").removeClassWild("special-" + parent + "-*").classList.add("special-" + parent + "-" + document.querySelector(this).value);
    //           document.cookie = `special=${JSON.stringify($special)}; path=/;`;
    //           break;
    //         case "letter-spacing":
    //           $special.letter_spacing = parseInt(document.querySelector(this).value);
    //           document.querySelector(this).classList.add("active");
    //           document.querySelector("html").removeClassWild("special-" + parent + "-*").classList.add("special-" + parent + "-" + document.querySelector(this).value);
    //           document.cookie = `special=${JSON.stringify($special)}; path=/;`;
    //           break;
    ...
    //           document.querySelector(this).value = $special.images;
    ...
    //             document.querySelector("i.special-audio").remove();
    //             if (responsiveVoice.isPlaying()) responsiveVoice.cancel();
    //             document.querySelector("p,h1,h2,h3,h4,h5,h6,li,dt,dd,.audiotext").off();
    //             document.querySelector(this).value = 0;
    //           } else {
    //             responsiveVoice.speak("Включено озвучивание текста.", "Russian Female");
    //             document.querySelector(this).classList.add("active");
    //             document.querySelector(this).value = 1;
